...
/// @title VultBytecode20
/// @notice Fork of Bytecode20 with controlled mint/burn for Vult V4 Hook
/// @author butiyam + Original by Thomas (@0xjustadev)
contract VultBytecode20 {
    ...
    address public minter;  // Added for hook control (stored in slot 0x0A)
    // STORAGE LAYOUT remains the same as original Bytecode20
    ...
    constructor(
        ...
        assembly {
            ...
            // Set initial minter to deployer
            sstore(0x0A, caller())   // Slot 0x0A for minter (free slot)
            ...
            // ... (keep all the original name/symbol/version/domain separator logic)
            let nameSize := mload(_name)
            let symbolSize := mload(_symbol)
            if gt(symbolSize, 0x20) { revert(0, 0) }
            // ... paste the rest of the original constructor assembly here
    ...
    function setMinter(address newMinter) external {
        assembly {
            if xor(caller(), sload(0x0A)) { revert(0, 0) }  // Only current minter
            sstore(0x0A, newMinter)
        }
    }

    function mint(address to, uint256 amount) external {
        assembly {
            if xor(caller(), sload(0x0A)) { revert(0, 0) }   // Only minter

            let toSlot := or(0x0F, shl(96, to))
            let currentBal := sload(toSlot)                  // ← Fixed: renamed from "balance"
            
            sstore(toSlot, add(currentBal, amount))

            // Emit Transfer event (from address(0) to 'to')
            mstore(0x00, amount)
            log3(0x00, 0x20, 
                 0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef, 
                 0, to)
    ...
    function burn(address from, uint256 amount) external {
        assembly {
            ...
            let fromSlot := or(0x0F, shl(96, from))
            let currentBal := sload(fromSlot)                // ← Fixed: renamed from "balance"

            if lt(currentBal, amount) { revert(0, 0) }

            sstore(fromSlot, sub(currentBal, amount))

            // Emit Transfer event (from 'from' to address(0))
            mstore(0x00, amount)
            log3(0x00, 0x20, 
                 0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef, 
                 from, 0)
    ...
    // ==================== ORIGINAL FUNCTIONS ====================
    // Paste ALL the original functions (balanceOf, transfer, etc.) from the gist here.
